...
def main(fconfig):
    ...
    output_file =  fconfig['obs']['romsobs']['CMEMS']['output_file']
    
    #end_day = pd.Timestamp.today()
    #start_day     = pd.Timestamp.today() - pd.Timedelta(days=4)
    ...
    for day in days:
        ...
        outside = np.where(np.isnan(All['obs_Xgrid']) | np.isnan(All['obs_Ygrid']) |
                    ...
                    (All['obs_depth'] < 0) | (All['obs_value'] == 0))[0]  
        ...
        z_r = np.vstack((-999999 * np.ones((1, M)), z_r, 999999 * np.ones((1, M))))

        # Interpolate vertical grid index at observation depths
        Zgrid = np.full((maxDepth, M), np.nan)
        for i in range(M):
            interp_func = interp1d(z_r[:, i], np.arange(grd['N'] + 2), bounds_error=False, fill_value=np.nan)
