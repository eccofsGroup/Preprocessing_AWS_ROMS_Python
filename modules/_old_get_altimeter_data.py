...
def get_rads_ssh(rads_dir):
    ...
    """
    # 1) selection window
    sel_start = np.zeros((), dtype='datetime64')
...
def get_altimeter_data(grd, alt_dir, start_day):
        ...
        for idx in inds:
           ...
           # --- 1) smoothing ---
            if len(ssh) >= 8:
                ...
                ssh = np.convolve(ssh, np.ones(8)/8, mode='same')
                # loess
                frac = 8.0 / len(ssh)
                ssh = lowess(ssh, np.arange(len(ssh)),
    ...
    return data
#-------------------------------------------------------------------------
def get_all_altimetry(grd,altimeter_data,start_day,fconfig):
