...
def main(fconfig):
    ...
    end_day = np.datetime64('now','D') - np.timedelta64(1,'D')
    start_day     = end_day - np.timedelta64(ndays,'D')
    ...
    if all_files:
        ...
        if (start_day> ref +np.timedelta64(times[-1],'D')):
           start_day = ref + np.timedelta64(times[-1],'D')
               
    else:
        start=(np.floor((np.datetime64('now', 's')- ref) / np.timedelta64(1, 'D')))-6 
                   
        
            #altimeter_data = np.load('altimeter_data.npz')
    
    if True:
        altimeter_data = get_all_altimetry(grd, start,fconfig)
    else:
        altimeter_data =np.load(f"{working_dir}altimeter_data.npz")

    all_time = np.array(altimeter_data['time'])

    start_day_int=(start_day-ref)/ np.timedelta64(1, 'D')

    idxs = np.where(altimeter_data['time'] >  (start_day_int-10))[0]

    for key in altimeter_data.keys():
                altimeter_data[key] = np.array(altimeter_data[key])[idxs]
