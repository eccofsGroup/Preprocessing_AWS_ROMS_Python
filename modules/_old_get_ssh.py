...
def main(fconfig):
    ...
    plot_file       = fconfig['obs']['romsobs']['SSH']['plot_file']
    # ————————————————
    # TIME PARAMETERS
    # ————————————————
    ...
    ndays=fconfig['obs']['romsobs']['ndays']+1
    
    start_day  = (np.datetime64('now', 's')- np.timedelta64(ndays, 'D') - ref) / np.timedelta64(1, 'D') 
    
   # today = (np.datetime64('now', 's')- ref) / np.timedelta64(1, 'D') 

    end_day    = (np.datetime64('now', 's')- ref) / np.timedelta64(1, 'D') # or today
    ...
    if True:
        altimeter_data = get_all_altimetry(grd,altimeter_data,start_day,fconfig)
    
    altimeter_data = np.load(fconfig['obs']['romsobs']['SSH']['altfile']) 

    # ————————————————
    # FIND ALL ALONG-TRACK FILES
    # ————————————————
    all_files = glob.glob(os.path.join(working_dir, "along_track*.nc"))
    ...
    #  Loop over each day
    #for day in range(int(rtime.min()), int(rtime.max()) + 1):
    for day in range(int(rtime.min()), int(rtime.min()) + 3):
       ...
       # initialize output lists
        ...
        add_history(OBS_out, add_history)
