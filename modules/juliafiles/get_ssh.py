...
def get_ssh():
    ...
    working_dir     = "/p/julia/ROMS/eccofs/OBS_python/SSH/OBS_FILES_1h_16_3kmtide"
    ...
    end_day = (np.datetime64('now', 's')- ref) / np.timedelta64(1, 'D') 
    ...
    altimeter_data = get_all_altimetry(grd, start_day)
         
    # ---  SSHA standard‐deviation by interpolation ---
    print("Loading SSHA mean std")
    m1 = np.load('nodac_ssha_mean_std.npz')
    lon   = m1['lon']     # 2D array
    lat   = m1['lat']
    essh  = m1['essh']
    ...
    f = np.load('eccofs_woa_mercator_mdt.npz')
